...
{      
  ...
  while (EraseEndAddress>=EraseStartAddress)
  {
    //sFLASH_EraseSector(EraseStartAddress);
      SPI_FLASH_SectorErase(EraseStartAddress);
...
uint32_t CheckSum(uint32_t StartAddress, uint32_t Size, uint32_t InitVal)
{
  ...
  for(cnt=0; cnt<Size ; cnt+=4)
  {
    //sFLASH_ReadBuffer(&value, StartAddress ,1);
      SPI_FLASH_BufferWrite(&value, StartAddress ,1);
    Val = value;
    //sFLASH_ReadBuffer(&value, StartAddress + 1,1);
      SPI_FLASH_BufferWrite(&value, StartAddress + 1,1);
    Val+= value<<8;
    //sFLASH_ReadBuffer(&value, StartAddress + 2,1);
      SPI_FLASH_BufferWrite(&value, StartAddress + 2,1);
    Val+= value<<16;
    //sFLASH_ReadBuffer(&value, StartAddress + 3,1);
      SPI_FLASH_BufferWrite(&value, StartAddress + 3,1);
    Val+= value<<24;
...
uint64_t Verify (uint32_t MemoryAddr, uint32_t RAMBufferAddr, uint32_t Size, uint32_t missalignement)
{
  ...
  while (Size>VerifiedData)
  {
    //sFLASH_ReadBuffer(&TmpBuffer, MemoryAddr+VerifiedData, 1);
         SPI_FLASH_BufferWrite(&TmpBuffer, MemoryAddr+VerifiedData, 1);
